...
class FaceLockSystem:
    ...
    def process_frame(self, frame_bgr: np.ndarray) -> Dict:
        ...
        # Detect ALL faces (not just 1)
        detected_faces = self.detector.detect(frame_bgr, max_faces=10)
        ...
        result = {
            ...
            "all_faces": [],  # NEW: all detected faces with recognition
        }
        ...
        # If target face found, use it for state machine
        if target_face:
            ...
            # Target face NOT found - go to LOST state
            if self.state.state == self.state.LOCKED:
                self.state.frames_since_detection += 1
                if self.state.frames_since_detection > self.lock_timeout_frames:
                    self.state.state = self.state.SEARCHING
